...
2. Register the client's intake: business type, target public function,
   prior filing history in Colombia if any.
3. Run the advisor in read-only mode against SECOP II (Sistema Electrónico de Contratación Pública), managed by Colombia Compra Eficiente.
...
- named referral relationship with Colombian-licensed counsel or a
...
a working referral relationship with Colombian-licensed counsel or a
registered agent for whatever licensed representation the law of
Colombia requires for actual public-procurement filings.
