# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Colombia

## Classification

- Repository: `cloud-itonami-iso3166-col`
- ISO 3166: `COL` (Colombia)
...
- Social impact: [:msme-regional-supplier-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Colombian
  public contract
...
- registration walkthrough for SECOP II (Sistema Electrónico de Contratación Pública), managed by Colombia Compra Eficiente
- business/tax registration checklist: a RUT (Registro Único Tributario) tax registration plus a RUP (Registro Único de Proponentes) supplier registration via the local chamber of commerce; VAT (IVA) registration with the DIAN (Dirección de Impuestos y Aduanas Nacionales)
- local-content / preferential-procurement navigation: no blanket local-content quota, but SECOP II's evaluation criteria typically weight MSME/regional supplier participation on smaller tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Colombian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Colombia).
