(ns marketentry.marketentryllm
  ...
  the COL public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-co-entity?=" (:has-co-entity? e)
                        " nit-verified?=" (:nit-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-co-entity? e))
                              (:has-co-entity? e))
                          (or (not (:requires-nit? e))
                              (:nit-verified? e)))
                   0.9 0.3)}))
