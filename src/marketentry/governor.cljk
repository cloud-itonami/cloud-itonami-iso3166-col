(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Colombia-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Colombia-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-co-entity?
                                       true` (near-universal for Colombia
                                       ...
                                       Grounded in Colombian legal entity (NIT)
                                       typically required for SECOP awards
                                       (contracting authorities / CCE).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(NIT/統一資格/SECOP II登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- co-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-co-entity? true`, INDEPENDENTLY verify
  `:has-co-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-co-entity?` ground truth (most Colombia public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-co-entity? e))
                 (not (true? (:has-co-entity? e))))
        [{:rule :co-entity-missing
          :detail (str subject " はコロンビア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- nit-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-nit? true`, INDEPENDENTLY check
  `:nit-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-nit? e))
                 (not (true? (:nit-verified? e))))
        [{:rule :nit-unverified
          :detail (str subject " はNIT確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (co-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (nit-unverified-violations request st)
